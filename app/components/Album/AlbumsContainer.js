...
import { QUERY_PUBLIC_ALBUMS, QUERY_ALBUMS } from "../../utils/graphql";
...
function AlbumsContainer(props) {
  ...
  const QUERY = props.isPublic ? QUERY_PUBLIC_ALBUMS : QUERY_ALBUMS;

  const [getAlbums, { error, loading, data, fetchMore, networkStatus }] = useLazyQuery(QUERY, {
    variables: {
  ...
  useEffect(() => {
    if (data) {
      if (props.isPublic) {
        ...
        setAlbums(data.queryAlbums.edges);
        setEndCursor(data.queryAlbums.pageInfo.endCursor);
        setHasNextPage(data.queryAlbums.pageInfo.hasNextPage);
      }
    } else {
      setAlbums([]);
      setEndCursor("");
      ...
      {!loading &&
        ...
        props.username === user.username && (
          <h3 className="text-gray-500 text-center mt-5 mb-3 text-xs md:text-sm">
            You currently have no albums on display...
          </h3>
        )}
      {!loading && !user && debouncedSearchTerm.length === 0 && albums.length === 0 && (
        <h3 className="text-gray-500 text-center mt-5 mb-3 text-xs md:text-sm">
          {capitalizeFirstLetter(props.username)} currently have no albums on display...
        </h3>
      )}
